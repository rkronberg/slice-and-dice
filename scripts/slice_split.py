...
def parse():
    ...
    p.add_argument('-i', '--input', required=True,
                   help='Input trajectory (.xyz)')
    g1 = p.add_mutually_exclusive_group(required=True)
    g1.add_argument('-n', '--nframes', type=int,
                    help='Number of frames in split subtrajectory')
    g1.add_argument('-ts', '--step', type=int,
                    help='New stepsize in coarse-grained trajectory')
    g2 = p.add_mutually_exclusive_group(required=True)
    g2.add_argument('-r', '--regex', type=str,
                    help='Regular expression to get frame size')
    g2.add_argument('-fs', '--fsize', type=int,
                    help='Number of lines per frame')
    p.add_argument('-f', '--first', default=0, type=int,
                   help='First frame to include')
    p.add_argument('-l', '--last', default=int(1e16), type=int,
...
def main():
    ...
    with open(file, 'r') as inp:
        # If fsize is not specified, tries to get number of lines per frame
        # (number of atoms + headers) based on given expression that repeats
        # regularly every frame (in .xyz files 'time' or 'frame' etc..)
